...
// MODAL TIMER & EVENT LISTENERS SETUP
function setupPreferenceModal() {

    const modal = 
        document.getElementById("travelPreferenceModal");

    const modalDismissed =
         sessionStorage.getItem("prefModalDismissed");

    if (modal && !modalDismissed) { 
    ...
    window.closePreferenceModal = function () {
        ...
        if (modal) {
            ...
            sessionStorage.setItem("prefModalDismissed", "true");

        }
    };

    // Form Submit Handler (Redirects to recommendation.html)
    window.handlePreferenceSubmit = function (event) {

        if (event) event.preventDefault();

        const preferences = {

            budget: document.getElementById("prefBudget")?.value || "Medium",

            travelType: document.getElementById("prefType")?.value || "Adventure",

            duration: document.getElementById("prefDuration")?.value || "5",

            travelers: document.getElementById("prefTravelers")?.value || "2",

            category: document.getElementById("prefCategory")?.value || "Mountains",

        };

        // Save preferences in SessionStorage
        sessionStorage.setItem("userTravelPreferences", JSON.stringify(preferences));

        // Modal Hide
        if (typeof window.closePreferenceModal === "function") {
...
// Ensure DOM is fully loaded before executing modal setup
if (document.readyState === "loading") {

    document.addEventListener("DOMContentLoaded", setupPreferenceModal);
